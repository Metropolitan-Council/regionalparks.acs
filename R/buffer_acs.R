## code to prepare `buffer_distances` dataset goes here
pkgload::load_all()
...
agency_boundary <- read_sf("./agencyboundaries/Park_Operating_Agencies.shp") %>%
...
agency_avg <- bg_temp %>%
  ...
  left_join(bg_temp %>% #merge back in with all block groups
              select(GEOID, bg_area)) %>%
...
buffer_block_group_1.0mi_raw <- bg_temp %>%
...
buffer_block_group_1.5mi_raw <- bg_temp %>%
...
buffer_block_group_3mi_raw <- bg_temp %>%
