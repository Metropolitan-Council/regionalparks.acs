...
mod_leaflet_ui <- function(id) {
  ns <- NS(id)
  tagList(
    leafletOutput(ns("map"), width = "100%", height = 750)
  )
}
...
mod_leaflet_server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>% 
      ...
      addPolygons(data = park_trail_geog$park,
                  ...
                  highlightOptions = highlightOptions(stroke = TRUE, color = "white", weight = 2,
                                                      bringToFront = TRUE),
      ...
      addPolylines(data = park_trail_geog$trail,
