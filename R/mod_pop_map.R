...
mod_pop_map_server <- function(input, output, session,
  ...
  output$popmap <- mod_leaflet_base_server(
    input = input,
    output = output,
    session = session
  ) #----
  ...
  observeEvent(selected_popvars$input_pop, {
    ...
    leafletProxy("popmap") %>%
      ...
      addLegend(
        title = paste0(filter(popkey, popvar == selected_popvars$input_pop) %>% select(short)),
        position = "bottomleft",
        group = "Population data",
    ...
    leafletProxy("popmap") %>%
      addPolygons(
        ...
        popup = case_when(
          ...
          (selected_popvars$input_pop == "popdens_2040_mi" | selected_popvars$input_pop == "PopDens_2019") ~
          paste0(
            tags$strong(filter(popkey, popvar == selected_popvars$input_pop) %>% select(goodname)),
