...
```{r rawdata, message=FALSE, warning = F, echo = F}
...
```{r weighteddata, message=FALSE, warning = F, echo = F}
...
name_helper <- as_tibble(names(as_tibble(block_group_map))) %>% 
  ...
  mutate(dropdownname = acscode %>%
           ...
           str_replace("meanhhinc", "Socioeconomic, Mean household income ($)")) %>%
  ...
  mutate(category = case_when(str_detect(dropdownname, "Race|Ethnicity") ~ "EthRace",
                              TRUE ~ "other"))
usethis::use_data(name_helper, overwrite = TRUE)

# 
# lapply(with(filter(name_helper, category == "EthRace"), split(dropdownname, acscode)), as.list)
# lapply(with(filter(name_helper, category == "EthRace"), split(acscode, dropdownname)), as.list)

with(filter(name_helper, category == "EthRace"), split(acscode, dropdownname))
