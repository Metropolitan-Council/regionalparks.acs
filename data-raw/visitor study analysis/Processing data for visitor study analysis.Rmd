---
title: "Processing data for visitor study demographics"
author: "Ellen"
...
```{r unitgeo}
source("./park_trail_geog_oversample.R")
...
```{r rawacsdata}
source("census_tract_raw_oversample.R")
```
...
buffer_acs_fxn <- function(df) { # intersect the buffer of x distance with the acs demographics
  buff_acs <- acs_temp %>%
    ...
    mutate(intersect_area = st_area(.)) %>% # create new column with shape area
    select(GEOID, agency, name,  intersect_area) %>% # only select columns needed to merge
    st_drop_geometry() %>% # drop geometry as we don't need it
    left_join(acs_temp %>% # merge back in with all block groups
      select(GEOID, AREA)) %>%
    ...
    as_tibble() %>%
    select(GEOID, agency, name, coverage)
  return(buff_acs)
}


park_trail_geog_temp <- park_trail_geog_oversample %>% # bind_rows(park_trail_geog, .id = "status") %>%
...
long_buffer_data_oversample <- long_buffer_data_bg %>% 
